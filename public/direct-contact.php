...
// Table: 'contacts', colonne is_read: 'read'
...
try {
    ...
    switch ($action) {
        ...
        case 'add':
            ...
            $stmt = $pdo->prepare(
                "INSERT INTO contacts (name, email, subject, message, `read`, created_at, updated_at)
                 VALUES (?, ?, ?, ?, 0, NOW(), NOW())"
            );
        ...
        case 'get':
            $stmt = $pdo->query(
                "SELECT id, name, email, subject, message, `read` as is_read, created_at
                 FROM contacts
        ...
        case 'mark-read':
            ...
            $stmt = $pdo->prepare(
                "UPDATE contacts SET `read` = 1, updated_at = NOW() WHERE id = ?"
            );
            $stmt->execute([$id]);
